...
- Comprehensive per-command documentation under `docs/commands/` (`instrument`, `compare`, `reproduce`, `scorecard`, `dependencies`, `feeds`) covering purpose, inputs, output artifacts, examples, and edge cases, plus `docs/output-schemas.md` documenting the `.fennec/` cache layout and canonical `DashboardArtifactEnvelope` JSON schema; `README.md` links to the new docs. In-tool `--help` descriptions expanded across all commands/options for clearer guidance without leaving the terminal (Release Readiness)
- `--help` now ends with a copy-pasteable `Examples:` section for the root command, every command, and the `feeds` subcommands, via `FennecLabs.Cli.HelpExamples` wrapping the recursive help action (Release Readiness)
